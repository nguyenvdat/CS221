...
def peekingMDP():
    ...
    return BlackjackMDP([4, 3, 12, 11], 1, 20, 1)
    # END_YOUR_CODE
...
class QLearningAlgorithm(util.RLAlgorithm):
    ...
    def incorporateFeedback(self, state, action, reward, newState):
        ...
        if newState is not None:
            target = reward + self.discount * max(self.getQ(newState, new_action) for new_action in self.actions(newState))
        else:
            target = reward
        for f, v in self.featureExtractor(state, action):
            self.weights[f] = self.weights[f] - self.getStepSize() * (prediction - target) * v
...
def blackjackFeatureExtractor(state, action):
    ...
    if counts is not None:
        ...
        for j, count in enumerate(counts):
            features.append(((action, j, count), 1))
    return features
    # END_YOUR_CODE
