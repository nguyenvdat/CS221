...
def create_chain_csp(n):
    ...
    for i in range(0, n - 1):
        csp.add_variable(variables[i + 1], [0, 1])
...
def create_nqueens_csp(n = 8):
    ...
    for i in range(0, n - 1):
        for j in range(i + 1, n):
            csp.add_binary_factor(variables[i], variables[j], lambda x, y: x != y and not(x + i == y + j) and not(x - i == y - j))
    # END_YOUR_CODE
    return csp
        ...
        if not self.mcv:
            ...
            for var in self.csp.variables:
                if var not in assignment: 
                    ...
                    var_val_left.append((count_valid_values, var))
            return min(var_val_left)[1] 
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    csp.add_variable(result, [s for s in range(maxSum + 1)])
    if len(variables) == 0:
        return result
    for i, X_i in enumerate(variables):
        A_i = ('sum', name, i)
        csp.add_variable(A_i, [(s, domain) for domain in csp.values[X_i] for s in range(maxSum + 1)])
        csp.add_binary_factor(A_i, X_i, lambda x, y: x[1] == y)
        if i == 0:
            csp.add_unary_factor(A_i, lambda x: x[0] == 0)
        else:
            prev_A_i = ('sum', name, i - 1)
            csp.add_binary_factor(A_i, prev_A_i, lambda x, y: x[0]  == y[0] + y[1])
    csp.add_binary_factor(result, A_i, lambda x, y: x == y[0] + y[1])
    return result
    # END_YOUR_CODE
    ...
    def add_quarter_constraints(self, csp):
        ...
        for request in self.profile.requests:
            if len(request.quarters) > 0:
                for quater in self.profile.quarters:
                    if not quater in request.quarters:
                        csp.add_unary_factor((request, quater), lambda cid: cid is None)
